...
- 搭建 FastAPI 后端项目结构。
- 使用 MySQL root 密码创建 `carbon_eye` 数据库，并建立楼栋、水电、预测结果、行为得分表。
- 将 `prediction_results.csv` 和 `behavior_summary(1).csv` 导入 MySQL。
- 前端数据从静态示例切换为 API 获取。
- 行为识别和位置识别后续再接入，目前页面只保留入口。
...
- 地址：`127.0.0.1:3306`
- 数据库：允许创建 `carbon_eye`
- root 密码：由用户提供，后续应写入本地 `.env`，不要提交到 git。
